...
public class PegVsOthers {
	...
	@SuppressWarnings("java:S106") // Suppress warning about 'System.out.println'
	private static void checkConditionForTime(MarketData data, Time time, Condition condition, String message) throws ExecutionException,
			InterruptedException {
		System.out.println(message);
		ChangeCheck changeCheck = ChangeCheck.condition(condition).marketFilter(STOCKS).windowLength(250).from(time).to(time);
