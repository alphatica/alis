...
public class WilliamsRStrategyTest {
	...
	public static void main(String[] args) throws ExecutionException, InterruptedException {
		...
		GreenLambdaExecutor<Double> executor = new GreenLambdaExecutor<>();
		if (RUN_OPTIMIZATION) {
	...
	private static void executeOnce(GreenLambdaExecutor<Double> executor, MarketData stooqData) {
		executor.submit(() -> checkParams(124, 0.41, stooqData, OPTIMIZATION_END_TIME, FINAL_TIME));
	}

	private static void executeOptimization(GreenLambdaExecutor<Double> executor, MarketData stooqData) {
	...
	private static Double checkParams(int length, double level, MarketData marketData, Time startTime, Time endTime) {
		WilliamsRStrategy williamsRStrategy = new WilliamsRStrategy(length, level, 0.01);
