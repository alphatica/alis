		...
		for(TimeMarketData marketData: data.listMarkets(STOCKS)) {
			...
			if (account.getPosition(marketData.getMarketName()) != null) {
				if (sellSignal && !buySignal) {
					orders.add(new Order(marketData.getMarketName(), Direction.EXIT, OrderSize.PROPORTION, 1.0, 0.0));
				}
			} else {
				if (buySignal && !sellSignal) {
					orders.add(new Order(marketData.getMarketName(), Direction.LONG, OrderSize.PROPORTION, 0.05, buyChange));
				}
			}
		}
		return orders;
