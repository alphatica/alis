...
public class DonchianStrategy implements Strategy {
    ...
    public List<Order> afterClose(TimeMarketDataSet allData, Account account) {
        if (drawChart) {
            String time = allData.listMarkets().getFirst().getTime().toString();
    ...
    private void checkStock(Account account, TimeMarketData marketData, TimeMarketDataSet allData, List<Order> orders) {
        double size = account.getPosition(marketData.getMarketName()).map(p -> p.getLastPrice() * p.getSize()).orElse(0.0);
        if (size > 0 && lowestClose.matches(marketData, allData)) {
            orders.add(new Order(marketData.getMarketName(), SELL, PROPORTION, 1.0, 0.0));
        }
        if (size < account.getNAV() * 0.05 && highestClose.matches(marketData, allData)) {
            double priority = orderPriority(marketData.getLayer(Layer.CLOSE));
            orders.add(new Order(marketData.getMarketName(), BUY, PROPORTION, 0.05, priority));
        }
    }
