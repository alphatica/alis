...
//			checkMarketSinceIPO(market);
	...
	private static void showStats(Market market, Time lowestTime, double lowestBeforeStart, TimeMarketData startData, int sessions, double maxDDAfterLow) {
		...
		System.out.printf("%s,%s,%.2f,%s,%.2f,%.0f,%d,%.2f,%.2f,%.2f",
				market.getName(), lowestTime, lowestBeforeStart, startData.getTime(), startData.getData(Layer.HIGH, 0), maxDDAfterLow, sessions, highest, lowest, last);
